...
\begin{document}
	...
	Najosnovnije rečeno, paralelno programiranje radi tako što deli program na više zadataka koji se izvršavaju istovremeno. Da bi bolje razumeli celu sliku ipak, moramo prvo bolje objasniti paralelnu obradu.
...
Paralelizam se često koristi uz tradicionalno sekvencijalno programiranje, a i zbog gore navedenih mana, postavlja se pitanje, koliko je potencijalno ubrzanje izvršavanja zadataka ako primenimo paralelizam? Zbog ovoga se pominje ranije pomenut Amdalov zakon, koji meri efikasnost izvršavanja programa koji koristi paralelizam. On kaže da celokupan dobitak zavisi od količine vremena prilikom kojeg se zapravo koristi paralelnost (ili druga vrsta ubrzanja izvršavanja operacija). Formula je: \\
\begin{equation*}
S_{max} = \frac{1}{(1 - p) + \frac{p}{s}}
\end{equation*}\\
Gde je $S_{max}$ maksimalna potencijalna efikasnost, $p$ procenat sistema koji je poboljšan, a $s$ deo vremena u kojem radi poboljšani deo sistema. Kada bi se ova formula iz 1967. primenila danas ipak, naizgled bi paralelnost izgledala kao da ne ostvaruje dovoljno dobru efikasnost, ali uz razvoj tehnologije ipak je slučaj drugačiji. Zbog toga je Džon Gustafson odlučio da ponovo ispita ovaj zakon i došao do svog zaključka, koji prikazuje efikasnost paralelizma tako što upoređuje rad istog zadatka na mašini sa jednim jezgrom:\\
\begin{equation*}
S_{max} = N + (1 - N) * s
\end{equation*}\\
...
Dekompozicija prvo razdvaja obradu na više zadataka koji se kasnije dele među procesima. Traži paralelizam i koliko može da ga iskoristi. Cilj je kreirati dovoljno zadataka da bi procesori ih istovremeno izvršavali, pri čemu je broj zadataka gornje ograničen sa mogućim ubrzanjem rada, a njihov broj u jednom trenutku može da varira i mogu nastajati dinamički.\par 
...
Orkestracija se bavi struktuiranjem komunikacije, imenovanjem podataka, sinhronizacijom i generalno organizacijom rada. Svrha je da smanji trošak vremena tokom komunikacije i sinhronizacije, sačuva strukturu podataka i njihove lokalne reference, postara se da se zadaci izvršavaju po određenom rasporedu tako da ne dođe do preopterećenja ili manjka zadataka koji se izvršavaju. \par 
Nakon ovog koraka, već imamo paralelni program, ali mapiranje je tu da reguliše koji procesi će biti izvršavani na istom procesu, kao i koji procesi će se izvršavati na nekom specifičnom procesoru generalno. 
...
\end{itemize}
	...
	\begin{thebibliography}{50}
		...
		\bibitem{totalview} TotalView \emph{What Is Parallel Programming?} by Bill Burns ,on-line at: https://totalview.io/blog/what-is-parallel-programming
			
		\bibitem{perforce} Perforce \emph{What Is Parallel Programming and Multithreading (Multithreaded Programming)?} by Richard Bellairs, online at: https://www.perforce.com/blog/qac/multithreading-parallel-programming-c-cpp
		
		
		\bibitem{zoboko} \emph{Hands-On Parallel Programming with C\# 8 and .NET Core 3: Build solid enterprise software using task parallelism and multithreading} by Shakti Tanwar, 2019
...
Principles of Parallel Computation.} Bucknell University, Lewisburg, 1995.
		...
		\bibitem{dataandtaskparallel} ZDNet, James Reinders. \emph{Understanding task and data parallelism.} on-line at: https://www.zdnet.com/article/understanding-task-and-data-parallelism-3039289129/
